...
## dummy data 1, logarithmic
test2 <- data.frame(year = c(1999, 2000, 2001, 2002, 2003, 2004, 2005, 2006, 2007, 2008,
                           2009, 2010),
                  abundance = c(2.7, 3, 2.5, 5, 5.4, 6.8,7.5, 7.4, 7.9, 8.3, 8.1, 8.0 ))
fakedata_expon<-function(startyear, Nyears, startPop, noise, startK, startR, breaks, changeK, changeR){
  ...
  #calculate Nt vector
  ## kathryn new
  Nt <- c(startPop) # Create a population vector with the starting population as entry 1
  for (i in 1:length(year)) {
    Nt1 <- Nt[i] * exp(r[i]) * noisevector[i]
    Nt <- c(Nt, Nt1)
  }
...
test3<-fakedata_expon(noise=5, changeK=25, changeR=25, breaks=list("1905", "1910"))
#######################
...
expfit <- function(data) {
  # Fit the exponential model
  model <- nls(stand.response ~ a * exp(b * year), data = data, start = list(a = 1, b = 0))
  
  # Extract relevant outputs: a (scale), b (rate), and their standard errors
  output <- c(min(data$year), # Year the analysis started on
              ...
              coef(model)[1], # a (scale)
              sqrt(diag(vcov(model)))[1], # Standard error for a (scale)
              coef(model)[2], # b (rate)
              sqrt(diag(vcov(model)))[2], # Standard error for b (rate)
              summary(model)$sigma, # Residual standard error
              summary(model)$rsq, # R-squared
              summary(model)$adjr2) # Adjusted R-squared
...
output_frame<-multiple_breakups(test2)
output_frame
pyramid_plot(test2, rsq_points=FALSE)
